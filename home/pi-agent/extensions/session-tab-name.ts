...
const SESSION_BANNER_EXTENSION = join(
  homedir(),
  ".pi",
  "agent",
  "extensions",
...
export function sessionBannerAvailable(): boolean {
  const extensionPath =
    process.env.PI_SESSION_BANNER_EXTENSION_PATH || SESSION_BANNER_EXTENSION;
...
export default function sessionTabNameExtension(pi: ExtensionAPI) {
  // session-banner already provides richer AI names and Herdr synchronization.
  // Remain a portable fallback for computers where that optional extension is
  // absent, without racing it when both configurations are installed.
  if (
