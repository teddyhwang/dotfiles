...
import {
  ...
  panesInTab,
  renameHerdrTab,
  sessionBannerAvailable,
  toHerdrLabel,
} from "../home/pi-agent/extensions/session-tab-name.ts";

process.env.PI_SESSION_TAB_NAME_FORCE_FALLBACK = "1";
...
test("defers to session-banner when that optional extension is present", () => {
  const previousPath = process.env.PI_SESSION_BANNER_EXTENSION_PATH;
  const previousForce = process.env.PI_SESSION_TAB_NAME_FORCE_FALLBACK;
  process.env.PI_SESSION_BANNER_EXTENSION_PATH = path.resolve(
    "tests/session-tab-name.test.mjs",
  );
  delete process.env.PI_SESSION_TAB_NAME_FORCE_FALLBACK;

  try {
    assert.equal(sessionBannerAvailable(), true);
    sessionTabNameExtension({
      on() {
        assert.fail("fallback extension registered alongside session-banner");
      },
    });
  } finally {
    if (previousPath === undefined) {
      delete process.env.PI_SESSION_BANNER_EXTENSION_PATH;
    } else {
      process.env.PI_SESSION_BANNER_EXTENSION_PATH = previousPath;
    }
    if (previousForce === undefined) {
      delete process.env.PI_SESSION_TAB_NAME_FORCE_FALLBACK;
    } else {
      process.env.PI_SESSION_TAB_NAME_FORCE_FALLBACK = previousForce;
    }
  }
});
